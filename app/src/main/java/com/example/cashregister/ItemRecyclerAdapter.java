...
public class ItemRecyclerAdapter extends RecyclerView.Adapter<ItemRecyclerAdapter.ViewHolder> {
    ...
    public interface OnItemClickListener {
        void onItemClicked(SoldItem item);
    }

    private final OnItemClickListener listener;

    public ItemRecyclerAdapter(ArrayList<SoldItem> soldItemList, Context context, OnItemClickListener listener) {
    ...
    public ItemRecyclerAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(mContext)
                .inflate(R.layout.sold_item_recycler_list, parent, false);
    ...
    public void onBindViewHolder(@NonNull ItemRecyclerAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.getItemName().setText(soldItemList.get(position).getName());
        holder.getItemPrice().setText(String.format("%.2f", soldItemList.get(position).getTotal()));
        holder.getItemQuantity().setText(String.valueOf(soldItemList.get(position).getQuantity()));
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                listener.onItemClicked(soldItemList.get(position));
            }
        });
    }
